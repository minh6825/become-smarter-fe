...
const QuizItem = ({ quiz }: Props) => {
  return (
    <div className={`${keyframe['quiz-item']} quiz-item border gap-2 flex flex-col p-4 my-2 rounded-md shadow-xl cursor-pointer`}>
      ...
      <Link className="mt-auto" href={`/quiz/${quiz.quizId}`} key={quiz.quizId}>
        <ButtonPrimary type="button">Xem chi tiết</ButtonPrimary>
      </Link>
      </div>
  );
};
